...
object DauApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //从读取到的kafka数据，ds流转换为带有偏移量的特质类型，并获取到offset
   ...
   // filteredDStream.count().print()
    ...
    //TODO 功能3 向ES中保存日活数据
    filteredDStream.foreachRDD{ //行动算子
      rdd=>{//获取DS中的RDD（RDD是数据集，封装的大量的json对象数据）
        rdd.foreachPartition{//以分区为单位对RDD中的json对象数据进行批量处理
          jsonItr=>{
